...
public class MutateOperator
   ...
   * @since 3.0
   */
  public MutateOperator(final Configuration a_conf)
      throws InvalidConfigurationException {
    super(a_conf);
        ...
        if (mutate) {
          ...
          if (genes[j] instanceof ICompositeGene) {
            ...
            for (int k = 0; k < compositeGene.size(); k++) {
        	  ...
        	  else mutateGenePattern(genes[j]);
        	  if (m_monitorActive) {
  ...
  private void mutateGenePattern(final Gene a_gene){
	final String alphabet = "HCRWM";
  	final int N = alphabet.length();
  	Random r = new Random();
  	a_gene.setAllele(Character.toString(alphabet.charAt(r.nextInt(N))));
  }
  ...
  public int compareTo(Object a_other) {
    ...
    MutateOperator op = (MutateOperator) a_other;
    if (m_mutationRateCalc == null) {
